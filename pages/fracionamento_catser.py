...
COL_VALOR_EMPENHADO_ORIG = "Unnamed: 3"  # origem do valor empenhado
...
def carregar_dados_limite():
    ...
    # Renomeia Valor Empenhado se existir
    if COL_VALOR_EMPENHADO_ORIG in df.columns:
        df = df.rename(columns={COL_VALOR_EMPENHADO_ORIG: "Valor Empenhado"})
    else:
        df["Valor Empenhado"] = 0.0

    # Garante colunas básicas
    for c in ["CATSER", "Descrição"]:
        if c not in df.columns:
            df[c] = ""

    # CATSER como texto com 5 dígitos (zeros à esquerda)
    df["CATSER"] = (
        df["CATSER"]
        .astype(str)
        .str.replace(r"\.0$", "", regex=True)   # se vier como 123.0
        .str.replace(r"\D", "", regex=True)     # remove não dígitos
        .str.zfill(5)
    )

    # Trata Valor Empenhado numérico
    df["Valor Empenhado"] = (
        df["Valor Empenhado"]
        .astype(str)
    ...
    # Cria Limite da Dispensa fixo: R$ 65.492,11
    ...
    # Lista única de CATSER para o dropdown
    catsers_unicos = sorted(
        [
            c
            for c in df["CATSER"].dropna().unique()
            if isinstance(c, str) and c.strip() != ""
        ]
    )
    df._lista_catsers_unicos = catsers_unicos

    return df


df_limite_base = carregar_dados_limite()
CATSERS_UNICOS = getattr(df_limite_base, "_lista_catsers_unicos", [])
            ...
            children=[
                html.Div(
                    ...
                    children=[
                        ...
                        # Filtro CATSER (dropdown simples)
                        html.Div(
                            style={"minWidth": "220px", "flex": "1 1 260px"},
                            children=[
                                html.Label("CATSER"),
...
def atualizar_tabela_limite_itajuba(catser_texto, catser_drop):
    ...
    if catser_drop:
        dff = dff[dff["CATSER"] == catser_drop]
